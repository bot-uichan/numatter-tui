...
export class NumatterClient {
  ...
  createPost(input: { content?: string }): Promise<{ post: { id: string; content: string | null; createdAt: string } }> {
    const form = new FormData();
    if (input.content !== undefined) form.append("content", input.content);
    return this.request("POST", "/v1/posts", { formData: form });
  }

  getNotifications(input?: { type?: NotificationFilter; markAsRead?: boolean }): Promise<{ items: Array<{ id: string; type: string; createdAt: string; actor?: { name?: string }; sourcePost?: { content?: string | null } }>; unreadCount: number }> {
